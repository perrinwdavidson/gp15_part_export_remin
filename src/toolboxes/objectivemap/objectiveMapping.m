function [VhatEstMean, VhatVar, lx] = objectiveMapping(X, V, Xq, lenScaleBound, VVar) 
    ...
    %   variance ::
    %%% calculate ::
    % VhatVar = diag((VhatVarNorm .* Vstd) + Vmean);  % bug #7: multiplies by std (not std^2) and adds mean to variance; Var(V) = Var(z)*std^2, not Var(z)*std + mean
    VhatVar = diag(VhatVarNorm .* (Vstd .^ 2));
