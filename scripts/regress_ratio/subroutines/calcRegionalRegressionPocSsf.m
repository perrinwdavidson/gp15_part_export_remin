...
for istat = 1 : size(regions, 1)
    ...
    for idat = 1 : length(idx_stat)
        ...
        idx_obs = find((gp15_obs.stationNo == stat)                       & ...
                       ...
                       (~isnan(gp15_obs.pocSmall))                        & ...
                       (gp15_obs.pocTh234RatioSmall <= maxPocSsf)         & ...
                       (gp15_obs.pocTh234RatioSmall >= minPocSsf));
        if isempty(idx_obs), continue; end
        data_all = [data_all; ...
            gp15_obs.depth(idx_obs), ...
            gp15_obs.pocTh234RatioSmall(idx_obs), ...
            gp15_obs.uncertPocTh234RatioSmall(idx_obs), ...
            repmat(double(stat), numel(idx_obs), 1)];
    end
    data_all = rmmissing(data_all);

    %  fit piecewise model via profile likelihood ::
    %  n.b.: region 2 z* override removed — profile likelihood now determines
    %        the transition depth for all regions without exception
    % if istat == 2
    ...
    set(gca, 'box', 'on', 'yDir', 'reverse', ...
        'xLim', [0, 4], 'yLim', [0, BTMDEPTH], 'tickLabelInterpreter', 'latex');
...
exportgraphics(gcf, [plot_output_basepath 'regressRegionalRatio/regressionPocSsf.pdf'], 'ContentType', 'vector');

%% save final result
%  make table ::
ratioPocSsf = array2table([regress_coeffs, deep_mean, mld_region, zstar_region, zstar_ci, ez_region]);
ratioPocSsf = [region_names', ratioPocSsf];
ratioPocSsf.Properties.VariableNames = {'region', 'ratioBeta', 'ratioAlpha', 'betaVar', 'alphaVar', 'coVar', 'n', ...
