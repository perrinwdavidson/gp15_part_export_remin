...
%  set bounds ::
BTMDEPTH = 400; 
maxPocSsfPpz = 100;  %3.0;
maxPocSsf400 = 100;  %3.0; 
minPocSsfPpz = 0; %0.05;
minPocSsf400 = 0; %0.05; 
...
for istat = 1 : 1 : size(regions, 1)
	...
	for idat = 1 : 1 : length(idx_stat)
		...
		idx_ppz = find((gp15_obs.stationNo == stat) & (gp15_obs.depth >= stat_mld) & (gp15_obs.depth < stat_ppz) & (~isnan(gp15_obs.pocSmall)) & (gp15_obs.pocTh234RatioSmall <= maxPocSsfPpz) & (gp15_obs.pocTh234RatioSmall >= minPocSsfPpz));
		idx_400 = find((gp15_obs.stationNo == stat) & (gp15_obs.depth >= stat_ppz) & (gp15_obs.depth < BTMDEPTH) & (~isnan(gp15_obs.pocSmall)) & (gp15_obs.pocTh234RatioSmall <= maxPocSsf400) & (gp15_obs.pocTh234RatioSmall >= minPocSsfPpz));
		data_ppz = [data_ppz; [gp15_obs.depth(idx_ppz), gp15_obs.pocTh234RatioSmall(idx_ppz), gp15_obs.uncertPocTh234RatioSmall(idx_ppz)]];
		data_400 = [data_400; [gp15_obs.depth(idx_400), gp15_obs.pocTh234RatioSmall(idx_400), gp15_obs.uncertPocTh234RatioSmall(idx_400)]];
	...
	set(gca, 'box', 'on', 'yDir', 'reverse', 'xLim', [0, 4], 'yLim', [0, 400], 'tickLabelInterpreter', 'latex'); 

end

%  set labels ::
xlabel(tl, 'SSF POC:$^{234}$Th [$\mu$mol dpm$^{-1}$]', 'interpreter', 'latex'); 
ylabel(tl, 'Depth [m]', 'interpreter', 'latex');
set(gcf, 'position', [0, 0, 1200, 400]);
exportgraphics(gcf, [plot_output_basepath 'regressRegionalRatio/regressionPocSsf.png'], 'resolution', 300);

%% save final result
%  make data ::
ratioPocSsf = array2table([regress_coeffs, deep_mean, mld_region, zstar_region, ez_region]); 
ratioPocSsf = [region_names', ratioPocSsf]; 
ratioPocSsf.Properties.VariableNames = {'region', 'ratioBeta', 'ratioAlpha', 'betaVar', 'alphaVar', 'coVar', 'n', 'ratioSubsurface', 'ratioSubsurfaceStdErr', 'ratioSubsurfaceStd', 'mldMean', 'zstar', 'ppzmean'};

%  save ::
save([sim_output_basepath 'regressRegionalRatio/ratioPocSsf.mat'], 'ratioPocSsf'); 
writetable(ratioPocSsf, [sim_output_basepath 'regressRegionalRatio/ratioPocSsf.xlsx']); 
