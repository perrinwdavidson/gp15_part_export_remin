...
for istat = 1 : 1 : size(regions, 1)
	...
	for idat = 1 : 1 : length(idx_stat)
		...
		idx_ppz = find((gp15_obs.stationNo == stat) & (gp15_obs.depth >= stat_mld) & (gp15_obs.depth < stat_ppz) & (~isnan(gp15_obs.pocLarge)) & (gp15_obs.pocTh234RatioLarge <= maxPocLsfPpz) & (gp15_obs.pocTh234RatioLarge >= minPocLsfPpz));
		idx_400 = find((gp15_obs.stationNo == stat) & (gp15_obs.depth >= stat_ppz) & (gp15_obs.depth < BTMDEPTH) & (~isnan(gp15_obs.pocLarge)) & (gp15_obs.pocTh234RatioLarge <= maxPocLsf400) & (gp15_obs.pocTh234RatioLarge >= minPocLsfPpz));
		data_ppz = [data_ppz; [gp15_obs.depth(idx_ppz), gp15_obs.pocTh234RatioLarge(idx_ppz), gp15_obs.uncertPocTh234RatioLarge(idx_ppz)]];
		data_400 = [data_400; [gp15_obs.depth(idx_400), gp15_obs.pocTh234RatioLarge(idx_400), gp15_obs.uncertPocTh234RatioLarge(idx_400)]];
	...
	y_p_400{istat} = [repmat(deep_mean(istat, 1), 1, 1000) - deep_mean(istat, 2); repmat(deep_mean(istat, 1), 1, 1000) + deep_mean(istat, 2)]';  % use standard error of mean
	...
	set(gca, 'box', 'on', 'yDir', 'reverse', 'xLim', [0, 3], 'yLim', [0, 400], 'tickLabelInterpreter', 'latex'); 

	% make plotting data array ::
	if istat == 1

		% ppz ::
		plot_output_data.data_region = repmat(istat, length(data_ppz(:, 2)), 1); 
		plot_output_data.depth = data_ppz(:, 1); 
		plot_output_data.data = data_ppz(:, 2); 

		plot_output_fit.fit_region = repmat(istat, 1000, 1); 
		plot_output_fit.fit_depth = depth_ppz'; 
		plot_output_fit.fit_data = y_fit'; 
		plot_output_fit.fit_data_error_low = y_p_ppz{istat}(1, :)'; 
		plot_output_fit.fit_data_error_high = y_p_ppz{istat}(2, :)'; 

		% deep ::
		plot_output_data.data_region = [plot_output_data.data_region; repmat(istat, length(data_400(:, 2)), 1)]; 
		plot_output_data.depth = [plot_output_data.depth; data_400(:, 1)]; 
		plot_output_data.data = [plot_output_data.data; data_400(:, 2)]; 

		plot_output_fit.fit_region = [plot_output_fit.fit_region; repmat(istat, 1000, 1)]; 
		plot_output_fit.fit_depth = [plot_output_fit.fit_depth; depth_400']; 
		plot_output_fit.fit_data = [plot_output_fit.fit_data; repmat(deep_mean(istat, 1), 1000, 1)]; 
		plot_output_fit.fit_data_error_low = [plot_output_fit.fit_data_error_low; y_p_400{istat}(:, 1)];
		plot_output_fit.fit_data_error_high = [plot_output_fit.fit_data_error_high; y_p_400{istat}(:, 2)]; 

	else

		% ppz ::
		plot_output_data.data_region = [plot_output_data.data_region; repmat(istat, length(data_ppz(:, 2)), 1)]; 
		plot_output_data.depth = [plot_output_data.depth; data_ppz(:, 1)]; 
		plot_output_data.data = [plot_output_data.data; data_ppz(:, 2)]; 

		plot_output_fit.fit_region = [plot_output_fit.fit_region; repmat(istat, 1000, 1)]; 
		plot_output_fit.fit_depth = [plot_output_fit.fit_depth; depth_ppz']; 
		plot_output_fit.fit_data = [plot_output_fit.fit_data; y_fit']; 
...
exportgraphics(gcf, [plot_output_basepath 'regressRegionalRatio/regressionPocLsf.png'], 'resolution', 300);

%% save final result
%  make data ::
ratioPocLsf = array2table([regress_coeffs, deep_mean, mld_region, zstar_region, ez_region]); 
ratioPocLsf = [region_names', ratioPocLsf]; 
ratioPocLsf.Properties.VariableNames = {'region', 'ratioBeta', 'ratioAlpha', 'betaVar', 'alphaVar', 'coVar', 'n', 'ratioSubsurface', 'ratioSubsurfaceStdErr', 'ratioSubsurfaceStd', 'mldMean', 'zstar', 'ppzmean'};
