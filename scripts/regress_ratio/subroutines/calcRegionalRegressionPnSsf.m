...
for istat = 1 : size(regions, 1)
    ...
    for idat = 1 : length(idx_stat)
        ...
        idx_obs = find((gp15_obs.stationNo == stat)                      & ...
                       ...
                       (~isnan(gp15_obs.pnSmall))                        & ...
                       (gp15_obs.pnTh234RatioSmall <= maxPnSsf)          & ...
                       (gp15_obs.pnTh234RatioSmall >= minPnSsf));
        if isempty(idx_obs), continue; end
        data_all = [data_all; ...
            gp15_obs.depth(idx_obs), ...
            gp15_obs.pnTh234RatioSmall(idx_obs), ...
            gp15_obs.uncertPnTh234RatioSmall(idx_obs), ...
    ...
    set(gca, 'box', 'on', 'yDir', 'reverse', ...
        'xLim', [0, 1], 'yLim', [0, BTMDEPTH], 'tickLabelInterpreter', 'latex');
...
exportgraphics(gcf, [plot_output_basepath 'regressRegionalRatio/regressionPnSsf.pdf'], 'ContentType', 'vector');

%% save final result
%  make table ::
ratioPnSsf = array2table([regress_coeffs, deep_mean, mld_region, zstar_region, zstar_ci, ez_region]);
ratioPnSsf = [region_names', ratioPnSsf];
ratioPnSsf.Properties.VariableNames = {'region', 'ratioBeta', 'ratioAlpha', 'betaVar', 'alphaVar', 'coVar', 'n', ...
