	...
	for j = 1 : 1 : length(stats)
		...
		for k = 1 : 1 : length(depthsj)
			...
			if (dk >= 0) & (dk <= mldi)
				...
				% 	             sqrt(((ratioPocLsf.betaVar(i) * (mldi ^ 2)) + ratioPocLsf.alphaVar(i) + (2 * mldi * ratioPocLsf.coVar(i))) / ratioPocLsf.n(i))]];  % bug: divides by n inside sqrt; SE of predicted value = sqrt(betaVar*x^2 + alphaVar + 2*x*coVar), no /n
				regressPocRatio = [regressPocRatio; [(ratioPocLsf.ratioBeta(i) * mldi) + ratioPocLsf.ratioAlpha(i), ...
							             sqrt((ratioPocLsf.betaVar(i) * (mldi ^ 2)) + ratioPocLsf.alphaVar(i) + (2 * mldi * ratioPocLsf.coVar(i)))]];
				% regressPnRatio = [regressPnRatio; [(ratioPnLsf.ratioBeta(i) * mldi) + ratioPnLsf.ratioAlpha(i), ...
				% 	           sqrt(((ratioPnLsf.betaVar(i) * (mldi ^ 2)) + ratioPnLsf.alphaVar(i) + (2 * mldi * ratioPnLsf.coVar(i))) / ratioPnLsf.n(i))]];  % bug: same /n issue
				regressPnRatio = [regressPnRatio; [(ratioPnLsf.ratioBeta(i) * mldi) + ratioPnLsf.ratioAlpha(i), ...
							           sqrt((ratioPnLsf.betaVar(i) * (mldi ^ 2)) + ratioPnLsf.alphaVar(i) + (2 * mldi * ratioPnLsf.coVar(i)))]];
			...
			elseif (dk > ratioPocLsf.mldMean(i)) & (dk <= zstari)
				...
				% 	             sqrt(((ratioPocLsf.betaVar(i) * (dk .^ 2)) + ratioPocLsf.alphaVar(i) + (2 * dk * ratioPocLsf.coVar(i))) / ratioPocLsf.n(i))]];  % bug: same /n issue
				regressPocRatio = [regressPocRatio; [(ratioPocLsf.ratioBeta(i) * dk) + ratioPocLsf.ratioAlpha(i), ...
							             sqrt((ratioPocLsf.betaVar(i) * (dk .^ 2)) + ratioPocLsf.alphaVar(i) + (2 * dk * ratioPocLsf.coVar(i)))]];
				% regressPnRatio = [regressPnRatio; [(ratioPnLsf.ratioBeta(i) * dk) + ratioPnLsf.ratioAlpha(i), ...
				% 	           sqrt(((ratioPnLsf.betaVar(i) * (dk .^ 2)) + ratioPnLsf.alphaVar(i) + (2 * dk * ratioPnLsf.coVar(i))) / ratioPnLsf.n(i))]];  % bug: same /n issue
				regressPnRatio = [regressPnRatio; [(ratioPnLsf.ratioBeta(i) * dk) + ratioPnLsf.ratioAlpha(i), ...
							           sqrt((ratioPnLsf.betaVar(i) * (dk .^ 2)) + ratioPnLsf.alphaVar(i) + (2 * dk * ratioPnLsf.coVar(i)))]];

			% (zstar, bottom] ::
			elseif (dk > zstari)
				regressPocRatio = [regressPocRatio; [ratioPocLsf.ratioSubsurface(i), ratioPocLsf.ratioSubsurfaceStd(i)]];
				regressPnRatio  = [regressPnRatio;  [ratioPnLsf.ratioSubsurface(i),  ratioPnLsf.ratioSubsurfaceStd(i)]];
			end
