...
for istat = 1 : 1 : size(regions, 1)
	...
	for idat = 1 : 1 : length(idx_stat)
		...
		idx_ppz = find((gp15_obs.stationNo == stat) & (gp15_obs.depth >= stat_mld) & (gp15_obs.depth < stat_ppz) & (~isnan(gp15_obs.pnLarge)) & (gp15_obs.pnTh234RatioLarge <= maxPnLsfPpz) & (gp15_obs.pnTh234RatioLarge >= minPnLsfPpz));
		idx_400 = find((gp15_obs.stationNo == stat) & (gp15_obs.depth >= stat_ppz) & (gp15_obs.depth < BTMDEPTH) & (~isnan(gp15_obs.pnLarge)) & (gp15_obs.pnTh234RatioLarge <= maxPnLsf400) & (gp15_obs.pnTh234RatioLarge >= minPnLsfPpz));
		data_ppz = [data_ppz; [gp15_obs.depth(idx_ppz), gp15_obs.pnTh234RatioLarge(idx_ppz), gp15_obs.uncertPnTh234RatioLarge(idx_ppz)]];
		data_400 = [data_400; [gp15_obs.depth(idx_400), gp15_obs.pnTh234RatioLarge(idx_400), gp15_obs.uncertPnTh234RatioLarge(idx_400)]];
	...
	set(gca, 'box', 'on', 'yDir', 'reverse', 'xLim', [0, 1.2], 'yLim', [0, 400], 'tickLabelInterpreter', 'latex'); 
...
exportgraphics(gcf, [plot_output_basepath 'regressRegionalRatio/regressionPnLsf.png'], 'resolution', 300);

%% save final result
%  make data ::
ratioPnLsf = array2table([regress_coeffs, deep_mean, mld_region, zstar_region, ez_region]); 
ratioPnLsf = [region_names', ratioPnLsf]; 
ratioPnLsf.Properties.VariableNames = {'region', 'ratioBeta', 'ratioAlpha', 'betaVar', 'alphaVar', 'coVar', 'n', 'ratioSubsurface', 'ratioSubsurfaceStdErr', 'ratioSubsurfaceStd', 'mldMean', 'zstar', 'ppzmean'};

%  save ::
save([sim_output_basepath 'regressRegionalRatio/ratioPnLsf.mat'], 'ratioPnLsf'); 
writetable(ratioPocLsf, [sim_output_basepath 'regressRegionalRatio/ratioPnLsf.xlsx']); 
