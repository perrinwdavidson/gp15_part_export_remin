...
for iTime = 1 : 1 : NUMTIME
    for iLon = 1 : 1 : NUMLON
        for iLat = 1 : 1 : NUMLAT
            ...
            % vProfCentral = rmmissing(squeeze(u_mercator.uo(iLon, iLat, :, iTime)));  % bug #1: loads zonal (u) instead of meridional (v) component
            vProfCentral = rmmissing(squeeze(v_mercator.vo(iLon, iLat, :, iTime)));
	    ...
	    zProf = depth(1 : numSamp); 
            ...
            if iLon == 1  % forward finite difference
                ...
                % cos(lat) converts zonal degree spacing to east-west arc length; deg2km returns metres because A is in metres ::
                % dx = repmat(deg2km(dxDeg, A), numSamp, 1);  % missing cos(lat) — overestimates du/dx by 1/cos(lat)
                dx = repmat(deg2km(dxDeg, A) .* cos(vLatCentral * pi/180), numSamp, 1);  % m
            ...
            elseif iLon == NUMLON  % backward finite difference
                ...
                % dx = repmat(deg2km(dxDeg, A), numSamp, 1);  % missing cos(lat)
                dx = repmat(deg2km(dxDeg, A) .* cos(vLatCentral * pi/180), numSamp, 1);  % m
