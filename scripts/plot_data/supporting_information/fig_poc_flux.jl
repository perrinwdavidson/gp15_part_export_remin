...
for i in 1 : 1 : num_stat
    ...
    ax = Axis(fig₄[k[2], k[1]],
              ...
              ygridvisible = false)
#     band!(ax, 
#           Point2.(Float64.(poc_flux .- poc_flux_err), Float64.(poc_depth)), 
#           Point2.(Float64.(poc_flux .+ poc_flux_err), Float64.(poc_depth)),
#           color = (ColorSchemes.GnBu_9[7], 0.5))
    scatterlines!(ax, Float64.(poc_flux), Float64.(poc_depth), color = :black, marker = :rect, markersize = 12.5) 
    ...
    if stat_no in [27, 29, 31]
        ...
        d2 = Float64.([4.0 gp15_mld[i, 2]; 4.0 uci])
...
poc = [PolyElement(color = (:white, 0.5), strokecolor = :transparent, points = Point2f[(0, 0.25), (0, 0.75), (1, 0.75), (1, 0.25)]),
...
Legend(fig₄[7, 4], [poc, uci_plot], ["POC", "UCI"], tellheight = false, tellwidth = false, labelsize = 30, patchsize = (35, 35))

Label(fig₄[0, 2:4], rich("POC Flux (μmol m", superscript("-2"), " d", superscript("-1"), ")"), fontsize = 30)
Label(fig₄[3:5, 0], "Depth (m)", fontsize = 30, rotation = π/2)

save("plots/gp15Model/supporting_information/fig_poc_flux.pdf", fig₄)
