...
scatterlines!(ax₂, lat, stat_data[end:-1:1, end-2], color = :black, linestyle = :dash)
barplot!(ax₂, lat, stat_data[end:-1:1, end-2] .- stat_data[end:-1:1, end-1], fillto = stat_data[end:-1:1, end-2] .+ stat_data[end:-1:1, end-1], color = :black, gap = 0.99, strokecolor = :black, strokewidth = 1)
...
r100 = [LineElement(color = :black, linestyle = :dash), MarkerElement(color = :black, marker = :circle), PolyElement(color = :black, strokecolor = :black, strokewidth = 1, points = Point2f[(0.49, 0.1), (0.49, 0.9), (0.51, 0.9), (0.51, 0.1)])] 
...
save("plots/gp15Model/manuscript/fig7.pdf", fig₇)
