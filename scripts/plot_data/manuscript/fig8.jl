...
markersizes = log10.(poc_data[:, 4]) .* 30
colors = poc_data[:, 3]  #  (poc_data[:, 3] ./ maximum(poc_data[:, 3])) .* 30

# sblabel = rich("log", subscript("10"), rich(" P"), subscript("POC"), "(z) (mmol m", superscript("-2"), " d", superscript("-1"), ")")
sblabel = rich(rich("P"), subscript("POC"), "(z) (mmol m", superscript("-2"), " d", superscript("-1"), ")")

scatter!(Point2f.(poc_data[len_poc_dat+1:end, 2], poc_data[len_poc_dat+1:end, 1]), color = colors[len_poc_dat+1:end], markersize = markersizes[len_poc_dat+1:end], colormap = cgrad(:GnBu_9, 15, categorical = true), strokewidth = 2, strokecolor = :black, colorrange = (0, 250))  # , marker = 'o')

sb = scatter!(Point2f.(poc_data[1:len_poc_dat, 2], poc_data[1:len_poc_dat, 1]), color = colors[1:len_poc_dat], markersize = markersizes[1:len_poc_dat], colormap = cgrad(:GnBu_9, 15, categorical = true), strokewidth = 0.5, colorrange = (0, 250))  # , marker = 'o')

# Colorbar(fig₈[1, 2], sb, label = sblabel, ticks = -1:0.5:2, height = 600/δ, tellheight = false)
Colorbar(fig₈[1, 2], sb, label = "Sampling depth, z (m)", height = 600/δ, tellheight = false, ticks = 0:50:250)

group_type = [MarkerElement(marker = :circle, color = :white, strokecolor = :black, strokewidth = sw, markersize = 12.48) for sw in [2, 0.5]]
axislegend(ax₁, group_type, ["This Study", "Previous Studies"], position = :lb)  # tellheight = true)

group_size = [MarkerElement(marker = :circle, color = :black, strokecolor = :transparent, markersize = ms) for ms in (log10.([5, 10, 50]) .* 30)]  # (([50, 100, 200] ./ maximum(poc_data[:, 3])) .* 30)]
axislegend(ax₁, group_size, ["  5", "  10", "  50"], [sblabel], position = :rt, rowgap = 10, padding = (10.0, 10.0, 10.0, 10.0))  # tellheight = true)

xlims!(ax₁, -180, -60)
ylims!(ax₁, -25, 65)
save("plots/gp15Model/manuscript/fig8.pdf", fig₈)
